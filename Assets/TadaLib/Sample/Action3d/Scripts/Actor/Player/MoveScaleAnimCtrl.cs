...
namespace TadaLib.Sample.Action3d.Actor.Player
{
        ...
        , IScaleChanger
    {
        ...
        void UpdateJumpStartAnim(float deltaTime)
        {
            ...
            if (GetComponent<IDataHolder>().IsJumpStartFrame)
            {
                // ジャンプ開始
                _jumpAnimTimer.TimeReset(_jumpAnimDurationSec);
        ...
        float _landingAnimStrengthBase = 0.03f;
        [SerializeField, Range(0.1f, 1.0f)]
        float _landingAnimDurationSec = 0.15f;

        [SerializeField, Range(0.0f, 0.2f)]
        float _jumpAnimStrengthBase = 0.08f;
        [SerializeField, Range(0.1f, 1.0f)]
        float _jumpAnimDurationSec = 0.4f;

        [SerializeField, Range(0.0f, 0.8f)]
        float _fastFallAnimMaxStrength = 0.5f;
