...
public partial class SimpleAnimationPlayable : PlayableBehaviour
{
    ...
    {
        public void Initialize(string name, float animSpeed, AnimationClip clip, WrapMode wrapMode)
        {
            m_StateName = name;
            m_Clip = clip;
            m_WrapMode = wrapMode;
        ...
        {
            return (float)m_Playable.GetTime() >= m_Playable.GetDuration();
        }
    ...
    private class StateManagement
    {
        ...
        {
            return m_States.FindIndex(s => s != null && s.enabled && !s.IsAnimEnd()) != -1;
        }
