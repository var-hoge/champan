...
public partial class SimpleAnimation: MonoBehaviour, IAnimationClipSource
{
    ...
    private void Initialize()
    {
        ...
        m_Animator.updateMode = m_AnimatePhysics ? AnimatorUpdateMode.Normal : AnimatorUpdateMode.Normal;
        ...
        if (m_States != null)
        {
            foreach (var state in m_States)
            {
                if (state.clip)
                {
                    m_Playable.AddClip(state.clip, state.name, state.speed);
                }
            }
        }
    ...
    private void EnsureDefaultStateExists()
    {
        ...
        {
            m_Playable.AddClip(m_Clip, kDefaultStateName, 1.0f);
            Kick();
        }
    }
