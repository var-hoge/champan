...
public partial class SimpleAnimationPlayable : PlayableBehaviour
{
    ...
    private StateInfo DoAddClip(string name, float speed, AnimationClip clip)
    {
        //Start new State
        StateInfo newState = m_States.InsertState();
        newState.Initialize(name, speed, clip, clip.wrapMode);
    ...
    public bool AddClip(AnimationClip clip, string name, float speed)
    {
        StateInfo state = m_States.FindState(name);
        if (state != null)
        {
    ...
    public bool Play(string name, float? speedRate = null)
    {
        StateInfo state = m_States.FindState(name);
        if (state == null)
        {
        ...
        if (speedRate.HasValue)
        {
            state.speed = speedRate.Value;
        }
    ...
    private StateInfo CloneState(int index)
    {
        ...
        StateInfo clone = DoAddClip(newName, original.speed, original.clip);
