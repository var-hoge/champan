...
class StateDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        // Calculate rects
        Rect clipRect = new Rect(position.x, position.y, position.width * 3.5f / 10.0f - 5, position.height);
        Rect nameRect = new Rect(position.x + position.width * 3.5f / 10.0f + 5, position.y, position.width * 3.5f / 10.0f - 5, position.height);
        Rect speedLabelRect = new Rect(position.x + position.width * 7.0f / 10.0f + 5, position.y, position.width * 1.5f / 10.0f, position.height);
        Rect speedRect = new Rect(position.x + position.width * 8.5f / 10.0f - 5, position.y, position.width * 1.5f / 10.0f, position.height);
        ...
        EditorGUI.LabelField(speedLabelRect, "速度");
