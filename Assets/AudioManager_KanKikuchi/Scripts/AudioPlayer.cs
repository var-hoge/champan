...
public class AudioPlayer {
  ...
  private float GetVolume() {
    // @memo: 倍率を先に 1 で止めてから基準を掛ける (champan の改変)
    //
    // AudioSource の音量は 1.0 で頭打ちになる。
    // この作品には倍率に 20 を渡している音があり、
    // 基準を 0.05 まで下げるまで頭打ちのまま鳴り続けていた。
    // 音量の設定を動かしても、それらの音だけ変わらなかった。
    //
    // 先に止めておけば、どの音も基準にそのまま比例する。
    // 一番大きいときの聞こえ方は、頭打ちのため今までと変わらない。
    return _baseVolume * Mathf.Clamp01(_volumeRate);
  }
